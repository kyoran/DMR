...
def main():
    ...
    hd_rgb_camera_bp.set_attribute('exposure_min_bright', '12.0')  # under-exposure，默认是10，越小越亮
    ...
    hd_dvs_camera_bp.set_attribute('sigma_positive_threshold', str(0.1))  # 白噪声（摄像头自身电器原件噪声）
    hd_dvs_camera_bp.set_attribute('sigma_negative_threshold', str(0.1))
    ...
    for step in range(1000):
        ...
        next_obs, reward, done, _ = env.step([0, 0.8])    # 高速公路用这个来测试，跑的快点
        ...
        if args.encoder_type == "pixelInputFusion":
            con_conv = agent.critic.encoder(input_obs, vis=True).\
                squeeze().cpu().detach().numpy()   # (256, 6, 6)
            con_conv = np.sum(con_conv, axis=0)    # (6, 6)
            con_conv = cv2.resize(con_conv, (args.rl_image_size, args.rl_image_size))   # (128, 128)
            con_conv = np.uint8((con_conv - np.min(con_conv)) / (np.max(con_conv) - np.min(con_conv)) * 255)
            con_conv = cv2.applyColorMap(con_conv, cv2.COLORMAP_HSV)
            con_conv = cv2.cvtColor(con_conv, cv2.COLOR_BGR2RGB)
            ax[1, 0].imshow(con_conv)
            ax[1, 0].set_title("InputFuse")

            superimposed_rgb = con_conv * alpha + next_obs["RGB-Frame"] * (1 - alpha)
            superimposed_rgb = np.uint8(superimposed_rgb)
            superimposed_dvs = con_conv * alpha + next_obs["DVS-Frame"] * (1 - alpha)
            superimposed_dvs = np.uint8(superimposed_dvs)
            ax[2, 0].imshow(superimposed_rgb)
            ax[2, 0].set_title("InputFuse")
            ax[2, 1].imshow(superimposed_dvs)
            ax[2, 1].set_title("InputFuse")

            # cv2.waitKey(10)

        elif args.encoder_type == "pixelConNewV4":
            ...
            superimposed_rgb = rgb_conv * alpha + hd_rgb_data['img'] * (1 - alpha)
            superimposed_rgb = np.uint8(superimposed_rgb)
            superimposed_dvs = dvs_conv * alpha + hd_dvs_data['img'] * (1 - alpha)
            superimposed_dvs = np.uint8(superimposed_dvs)
            superimposed_com = con_conv * alpha + hd_rgb_data['img'] * (1 - alpha)
            superimposed_com = np.uint8(superimposed_com)
            ax[2, 0].imshow(superimposed_rgb)
            ax[2, 0].set_title("rgb")
            ax[2, 1].imshow(superimposed_dvs)
            ax[2, 1].set_title("dvs")
            ax[2, 2].imshow(superimposed_com)
            ax[2, 2].set_title("com")
