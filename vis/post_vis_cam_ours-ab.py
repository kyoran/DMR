...
def parse_args():
    ...
    # parser.add_argument('--perception_type', default='DVS-Frame',
    ...
    # parser.add_argument('--encoder_type', default='pixelCarla098', type=str)
    ...
    # parser.add_argument('--encoder_type', default='pixelConNewV4_Repel', type=str)

    parser.add_argument('--SEED', default=111, type=int)
    # parser.add_argument('--SEED', default=222, type=int)        # highbeam-hardrain
...
def main():
    ...
    for step in range(1000):
        ...
        if step <= 340:   # jaywalk-midnight
        # if step <= 633:      # highbeam-hardrain
        # if step <= 600:      # highbeam-midnight
            continue
        ...
        if args.encoder_type == "pixelCarla098":
            h, _, conv = agent.critic.encoder(
                torch.Tensor(obs["perception"]).unsqueeze(0).to(device),
                vis=True
            )
            conv = calc_conv(args, conv, [hd_height, hd_width])
            superimposed_rgb = conv * alpha_rgb + hd_rgb_data['img'] * (1 - alpha_rgb)
            superimposed_rgb = np.uint8(superimposed_rgb)
            superimposed_dvs = conv * alpha_dvs + hd_dvs_data['img'] * (1 - alpha_dvs)
            superimposed_dvs = np.uint8(superimposed_dvs)

            ax[1, 0].imshow(conv)
            ax[1, 0].set_title("conv")
            ax[2, 0].imshow(superimposed_rgb)
            ax[2, 0].set_title("rgb")
            ax[2, 1].imshow(superimposed_dvs)
            ax[2, 1].set_title("dvs")

            saved_dir = f"{args.domain_name}_{args.selected_weather}_{args.perception_type}_{args.encoder_type}"
            ...
            mask_extent = ax[1, 0].get_window_extent().transformed(fig.dpi_scale_trans.inverted())
            ...
            plt.savefig(f"{saved_dir}/{step}-mask.jpg", bbox_inches=mask_extent)
        ...
        if args.encoder_type == "pixelInputFusion":
            ...
            superimposed_dvs = np.uint8(superimposed_dvs)
            superimposed_com_on_rgb = con_conv * alpha_rgb + hd_rgb_data['img'] * (1 - alpha_rgb)
            superimposed_com_on_rgb = np.uint8(superimposed_com_on_rgb)
            superimposed_com_on_dvs = con_conv * alpha_dvs + hd_dvs_data['img'] * (1 - alpha_dvs)
            superimposed_com_on_dvs = np.uint8(superimposed_com_on_dvs)
            ax[2, 0].imshow(superimposed_rgb)
            ax[2, 0].set_title("rgb")
            ax[2, 1].imshow(superimposed_dvs)
            ax[2, 1].set_title("dvs")
            ax[2, 2].imshow(superimposed_com_on_rgb)
            ax[2, 2].set_title("com+rgb")
            ax[2, 3].imshow(superimposed_com_on_dvs)
            ax[2, 3].set_title("com+dvs")
            ...
            com_mask_extent = ax[1,2].get_window_extent().transformed(fig.dpi_scale_trans.inverted())
            rgb_mask_sup_extent = ax[2,0].get_window_extent().transformed(fig.dpi_scale_trans.inverted())
            dvs_mask_sup_extent = ax[2,1].get_window_extent().transformed(fig.dpi_scale_trans.inverted())
            com_mask_rgb_sup_extent = ax[2,2].get_window_extent().transformed(fig.dpi_scale_trans.inverted())
            com_mask_dvs_sup_extent = ax[2,3].get_window_extent().transformed(fig.dpi_scale_trans.inverted())
            plt.savefig(f"{saved_dir}/{step}-rgb.jpg", bbox_inches=rgb_extent)
            plt.savefig(f"{saved_dir}/{step}-dvs.jpg", bbox_inches=dvs_extent)
            ...
            plt.savefig(f"{saved_dir}/{step}-com-mask.jpg", bbox_inches=com_mask_extent)
            plt.savefig(f"{saved_dir}/{step}-rgb-mask-sup.jpg", bbox_inches=rgb_mask_sup_extent)
            plt.savefig(f"{saved_dir}/{step}-dvs-mask-sup.jpg", bbox_inches=dvs_mask_sup_extent)
            plt.savefig(f"{saved_dir}/{step}-com-mask-rgb-sup.jpg", bbox_inches=com_mask_rgb_sup_extent)
            plt.savefig(f"{saved_dir}/{step}-com-mask-dvs-sup.jpg", bbox_inches=com_mask_dvs_sup_extent)

        # plt.savefig(f"./conv/{step}-conv.jpg")
