...
# if domain=="highbeam" and weather == "midnight":
#     min_reward_ylim, max_reward_ylim = -30, 80
# elif domain == "highbeam" and weather == "hard_rain":
#     min_reward_ylim, max_reward_ylim = -30, 80
# elif domain=="jaywalk" and weather == "midnight":
#     min_reward_ylim, max_reward_ylim = -30, 140
# elif domain=="jaywalk" and weather == "hard_rain":
#     min_reward_ylim, max_reward_ylim = -30, 70
# min_reward_ylim, max_reward_ylim = -30, 400     # crash
    ...
    for one_seed in seeds:
        ...
        with open(one_logfile+f"+{one_seed}.log") as file:
            ...
            for one_line in lines:
                if one_line.startswith("| train | E:"):
                    sss = one_line[:-1].split("|")
                    train_steps.append(
                        int(sss[3].strip().split("S: ")[1])
                    )
                    if one_logfile.__contains__("pixelConNew"):
                        train_rewards.append(
                            float(sss[5].strip().split("R: ")[1])# * 1.8
                        )
                    else:
                        train_rewards.append(
                            float(sss[5].strip().split("R: ")[1])
                        )
                elif one_line.startswith("reward_each_episode:"):
                    sss = one_line[:-1].split(":")
                    # print(np.array(eval(sss[1].strip()), dtype=np.float32).shape)
                    test_rewards.append(
                        np.array(eval(sss[1].strip()), dtype=np.float32)
                    )
                # elif one_line.startswith("->average_reward:"):
                #     sss = one_line[:-1].split(":")
                #     test_rewards.append(
                #         float(eval(sss[1].strip()))
                #     )
                elif one_line.startswith("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@at step:"):
                    sss = one_line[:-1].split(":")
                    test_steps.append(
                        eval(sss[1].strip())[0]
                    )
...
plt.savefig(f"./reward/reward2.svg", bbox_inches="tight", pad_inches=0.1)
plt.show()
